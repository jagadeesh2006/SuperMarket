...
@WebServlet("/GetTopCustomerByBills")
public class GetTopCustomerByBills extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			  ...
			  String query = "SELECT CUSTOMERS.CUSTOMER_NAME,COUNT(BILL.CUSTOMER_ID) FROM CUSTOMERS INNER JOIN BILL ON CUSTOMERS.CUSTOMER_ID=BILL.CUSTOMER_ID WHERE BILL.BILL_DATE>='"+ sqlFromDate +"' AND BILL.BILL_DATE <='"+ sqlToDate +"' GROUP BY BILL.CUSTOMER_ID ORDER BY COUNT(BILL.CUSTOMER_ID) DESC LIMIT 1";			  
